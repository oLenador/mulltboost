...
interface CircularLoaderProps {
  items: number
  completed: number
  currentProgress?: number // progresso do item atual (0-100)
  size?: number
  strokeWidth?: number
  children?: ReactElement
  showPercentage?: boolean
  gap?: number // espaço entre segmentos em graus
}
...
export function CircularLoader({
  ...
  const segments = useMemo(() => {
    ...
    return Array.from({ length: safeItems }, (_, index) => {
      ...
      return { index, startAngle, endAngle, segmentDegrees, progress, status }
    })
